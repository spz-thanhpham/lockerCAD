...
function SignInForm() {
  ...
  const handleCredentials = async (e: React.FormEvent) => {
    ...
    setLoading(false)
    if (res?.error) setCredError('Invalid email or password.')
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-sm border w-full max-w-sm p-8">
        ...
        {error && (
          <div className="mb-4 px-3 py-2 bg-red-50 border border-red-200 rounded text-xs text-red-600">
            {error === 'OAuthAccountNotLinked'
              ...
              : error === 'AccessDenied'
              ? 'Access denied. Your account may not be authorised.'
        ...
        <button
          ...
          disabled={msLoading}
          className="w-full flex items-center justify-center gap-3 border border-gray-300 rounded-lg py-2.5 px-4 text-sm font-medium text-gray-700 hover:bg-gray-50 active:bg-gray-100 disabled:opacity-50 transition-colors mb-4"
        >
          {/* Microsoft logo SVG */}
        ...
        {tab === 'admin' && (
          <form onSubmit={handleCredentials} className="space-y-4">
            ...
            <p className="text-xs text-center text-gray-400">
              Set via <code className="bg-gray-100 px-1 rounded">ADMIN_EMAIL</code> /{' '}
        ...
        {tab === 'magic' && (
          magicSent ? (
            <div className="text-center py-4">
              ...
              <p className="text-xs text-gray-400 mt-2">(Dev: check terminal for the link)</p>
            ...
            <form onSubmit={handleMagicLink} className="space-y-4">
              <div>
                ...
                <input type="email" required value={magicEmail}
              ...
              <p className="text-xs text-center text-gray-400">Requires SMTP in .env</p>
            </form>
          )
        )}
      </div>
